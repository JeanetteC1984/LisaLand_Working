...
- **Sticker System (300+ stickers)**: 20+ categories including Seasonal (snowman, jack-o-lantern, ghost, xmas-tree, etc.). All draggable with position persistence, resizable (scale 0.3x–3x). Stickers are saved per journal entry (not global). Available in Journal, Mind Map, and Calendar sections. Calendar has its own `calendarStickers` state persisted in localStorage.
- **Notes System**: Separate NOTES button (distinct from STICKERS) in journal, mind map, and calendar toolbars. Notes panel with 12 theme-adaptive note types (Primary, Secondary, Accent, Warm, Soft, Bright, Deep, Pop, Vivid, Muted, Dreamy, Glow). Note colors use CSS `color-mix()` with theme variables (`--cy-primary`, `--cy-secondary`, `--cy-accent`, `--cy-warm`) — automatically adapts to every theme. CSS classes: `sn-primary`, `sn-secondary`, etc. Supports image and video note uploads (base64 embedded). Note stickers use `note-image-` and `note-video-` type prefixes.
